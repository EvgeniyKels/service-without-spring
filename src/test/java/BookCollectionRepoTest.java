...
public class BookCollectionRepoTest {
    ...
    void insertNewBookWithCorrectFields() {
        BookEntity bookEntity = new BookEntity(null, "author", "name", "descriptor");
        ...
        assertEquals(bookEntity.getAuthor(), document.get(AUTHOR_COLUMN_NAME));
        assertEquals(bookEntity.getBookName(), document.get(BOOK_NAME_COLUMN_NAME));
        assertEquals(bookEntity.getDescription(), document.get(DESCRIPTION_COLUMN_NAME));
    ...
    void getBookByIdTest() {
        ...
        BookEntity bookFromDb = bookOpt.get();
        assertEquals(bookEntity.getId(), bookFromDb.getId());
        assertEquals(bookEntity.getBookName(), bookFromDb.getBookName());
        assertEquals(bookEntity.getAuthor(), bookFromDb.getAuthor());
        assertEquals(bookEntity.getDescription(), bookFromDb.getDescription());
    }

    @Test
    void update() {
        BookEntity bookEntityBeforeUpdate = new BookEntity(null, "author", "name", "descriptor");
        String uuid = bookCollectionRepo.insertNewBook(bookEntityBeforeUpdate);
        BookEntity bookEntityAfterUpdate = new BookEntity(uuid, "author_up", "name_up", "descriptor_up");
